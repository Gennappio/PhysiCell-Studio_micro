...
#                            b = background * (1-alpha) + alpha*rgba_float[2];
    ...
    def update_vtk_window(self):
        """Update the VTK window to reflect the current frame if it exists."""
        try:
            # Try to import vtk_view module
            import vtk_view
            
            # Get the global VTK viewer instance
            vtk_viewer = vtk_view.get_vtk_viewer()
            
            # Check if we have a VTK viewer
            if vtk_viewer is not None:
                # Update the current frame
                vtk_viewer.set_current_frame(self.current_frame)
                print(f"Updated VTK viewer to frame {self.current_frame}")
        except Exception as e:
            # Silently ignore errors, this means the VTK viewer isn't available
            pass
